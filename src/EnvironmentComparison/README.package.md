...
Choose **Environment A** and **Environment B**, select the comparison areas, then select **Compare metadata**. Published metadata is compared by default; including unpublished metadata is an explicit option. Formula, form, and view XML is normalized to ignore formatting-only differences. Forms are matched by UniqueName when available and otherwise by their System Form component ID. Environment-specific root grid object type codes are ignored for view Layout XML equality. The grid shows compact fingerprints and CSV export includes complete normalized definitions. Values longer than Excel's cell limit are preserved across numbered Environment A/B columns.
